...
const LabTable = () => {
  ...
  const openModal = (item) => {
    ...
    axios
      .get(`http://localhost:5000/api/Complaints/${item._id}`)
      .then((response) => {
    ...
    // Fetch data from a different path for selectedData
    axios
      .get(`http://localhost:5000/api/Complaints/${patientId}`) // Replace with your desired path
      .then((response) => {
        // Update the selectedData state with the data from the different path
        setSelectedData({ ...selectedData, ...response.data });
      })
      .catch((error) => {
        console.error("Error fetching data from the different path:", error);
      });
  };
          ...
          <tbody>
            {appointments.map((item) => (
              <tr key={item.date}>
                ...
                <td>
                  <button className="button-2" onClick={() => openModal(item)}>
                    View
                  </button>
                </td>
              </tr>
