      ...
      try {
        ...
        try {
          const existingDataResponse = await axios.get(
            `http://localhost:5000/api/v1/allexistingpatients-data/${patientId}`
          );
          console.log(
            "Data from second collection:",
          ...
          console.error(
            "Error fetching data from the second collection:",
            error
          );
        }
  ...
  const handleSave = async (editedAppointment) => {
    try {
      if (!editedAppointment.id) {
        console.error("Invalid appointment ID:", editedAppointment._id);
        return;
      }
  
      // Send a PUT request with the edited appointment data
      await axios.put(
        `http://localhost:5000/api/v1/combined-dataUpdate/${editedAppointment.id}`,
        editedAppointment
      );
    ...
    <>
      ...
      <div style={{ display: "flex" }}>
        <div className="container_6">
          ...
          <div onClick={handleBackClick} style={{marginLeft:"10vw"}} >
            < FaArrowCircleLeft  style={{fontSize:'40px',color:'#0d6efd'}}/>
            </div><table className="appointments-table_6">
              <thead></thead>
              <tbody>
            ...
            {editingAppointment && (
              <EditAppointmentForm
              appointment={editingAppointment}  // Ensure that this is the correct object
              onSave={handleSave}
